...
func (logStore *LogStore) Init(path string, db *Database) error {
	...
	buff := make([]byte, comm.INT32SIZE) // 读取当前的file id
	...
	buff = make([]byte, comm.UINT32SIZE)
	n, err = metaFile.Read(buff)
	if n == comm.UINT32SIZE {
...
func (logStore *LogStore) Append(options *WriteOptions, instanceId uint64, buffer []byte, fileIdStr *string) error {
	...
	len := comm.UINT64SIZE + bufferLen
	tmpBufLen := len + comm.INT32SIZE
	...
	util.EncodeUint64(tmpBuf, comm.INT32SIZE, instanceId)
	copy(tmpBuf[comm.INT32SIZE+comm.UINT64SIZE:], []byte(buffer))
	...
	ckSum := util.Crc32(0, tmpBuf[comm.INT32SIZE:], comm.CRC32_SKIP)
...
func (logStore *LogStore) Read(fileIdstr string, instanceId *uint64) ([]byte, error) {
	...
	tmpbuf := make([]byte, comm.INT32SIZE)
	...
	fileCkSum := util.Crc32(0, tmpbuf, comm.CRC32_SKIP)
	if fileCkSum != cksum {
	...
	*/
	...
	return tmpbuf[comm.UINT64SIZE:], nil
}
...
func (logStore *LogStore) RebuildIndexForOneFile(fileId int32, offset uint64,
	...
	for {
		buffer := make([]byte, comm.INT32SIZE)
		...
		if n != comm.INT32SIZE {
			needTruncate = true
			log.Error("read len %d not equal to %d, need truncate", n, comm.INT32SIZE)
			err = nil
			break
		}
		...
		if int64(len) > fileLen || len < comm.UINT64SIZE {
		...
		*nowInstanceId = instanceId
		...
		err = proto.Unmarshal(buffer[comm.UINT64SIZE:], &state)
		if err != nil {
			...
			log.Error("this instance buffer wrong, can't parse to acceptState, instanceid %d bufferlen %d nowoffset %d",
				instanceId, len-comm.UINT64SIZE, nowOffset)
			err = nil
			break
		}

		fileCkSum := util.Crc32(0, buffer, comm.CRC32_SKIP)
...
func (logStore *LogStore) DecodeFileId(fileIdStr string, fileId *int32, offset *uint64, cksum *uint32) {
	...
	util.DecodeUint64(buffer, comm.INT32SIZE, offset)
	util.DecodeUint32(buffer, comm.INT32SIZE+comm.UINT64SIZE, cksum)
}

func (logStore *LogStore) EncodeFileId(fileId int32, offset uint64, cksum uint32, fileIdStr *string) {
	buffer := make([]byte, comm.INT32SIZE+comm.UINT64SIZE+comm.UINT32SIZE)
	util.EncodeInt32(buffer, 0, fileId)
	util.EncodeUint64(buffer, comm.INT32SIZE, offset)
	util.EncodeUint32(buffer, comm.INT32SIZE+comm.UINT64SIZE, cksum)
...
func (logStore *LogStore) IncreaseFileId() error {
	...
	if n != comm.INT32SIZE {
		return fmt.Errorf("write len %d not equal to %d", n, comm.INT32SIZE)
	}
	ckSum := util.Crc32(0, buffer, 0)
	buffer = make([]byte, comm.UINT32SIZE)
	...
	if n != comm.UINT32SIZE {
		return fmt.Errorf("write len %d not equal to %d", n, comm.UINT32SIZE)
	}
