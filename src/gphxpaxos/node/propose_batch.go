...
func (pb *ProposeBatch) main() {
	...
	for queueLen := len(pb.queue); queueLen > 0; queueLen = len(pb.queue) {
		...
		proposal.notifier <- comm.Paxos_SystemError
	}
...
func (pb *ProposeBatch) Propose(value []byte, instanceId uint64, batchIndex uint32, ctx *smbase.SMCtx) error {
	if pb.isEnd {
...
func (pb *ProposeBatch) AddProposal(value []byte, instacneId uint64, batchIndex uint32,
	ctx *smbase.SMCtx, notifier chan error) {
...
func (pb *ProposeBatch) DoProposal(requests []*PendingProposal) {
	...
	var batchSMCtx = &smbase.BatchSMCtx{}

	for _, proposal := range requests {
		value := &comm.PaxosValue{}
	...
	ctx := &smbase.SMCtx{SMID: comm.BATCH_PROPOSE_SMID, PCtx: batchSMCtx}
